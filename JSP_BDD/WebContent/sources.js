$(document)
		.ready(
				function() {
					...
					// vers un ELEMENT PARENT STATIQUE, sans quoi on ne pourra pas agir
					// sur les éléments enfants après qu'ils aient été supprimés et
					// recréés
					...
					// Bouton CONFIRMER : ajouter une entrée au tableau de sociétés
					// (-> récupérer les infos du formulaire '.formAjouterSociete'
					// à l'intérieur du div 'collapseAjouterSociete')
					$("#collapseAjouterSociete").on("click","#btnConfirmerAjoutSociete",
							function() {

						// Le comportement par défaut d'un bouton submit
					...
					// Bouton MODIFIER EMPLOYE : ouvrir un modal
					// qui permettra un employé à la société
					$("#tblEmployes").on(
							"click",
							".btnAjouterEmploye",
							function() {
								$.ajax({
									...
									data : "IdSociete="
											+ $(this).attr("data-idSociete")
											+ "&btnAction=AjoutEmploye",
									success : function(code, status) {
										$("#modalAjoutEmploye").html(code)
									}
								})
							})
				})