...
//		Message d'info
	...
	/**
	 * Lire une entrée du tableau à partir de son ID_Point
	 * @param id
	 * @return
	 */
	public Rectangle2 ReadPoint(int id) {

		Rectangle2 output = null;
		String req_Rectangle = "SELECT * FROM Rectangle WHERE ID_Point = ?";

//		Message d'info
		System.out.println("DAO_Rectangle ReadPoint");

		try {
			PreparedStatement ps_Rectangle = _Cnn.prepareStatement(req_Rectangle);

			ps_Rectangle.setInt(1, id); // Complète la requête SQL
			System.out.println(ps_Rectangle);
			ResultSet rs_Rectangle = ps_Rectangle.executeQuery();
			...
			System.out.println("DAO_Rectangle ReadPoint() error: " + e.getMessage() + "\n");
		}
